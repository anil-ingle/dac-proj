	...
	let aid='';
					...
					jq(document).ready(function() {
						...
						jq("#city").change(function() {
							clrscr=true;
						var cityId = $(this).val();
						jq.get(ajaxPrefix+"mvc/cityArea?"+"cityId="+ cityId,function(data) {
						...
						jq("#area").change(function() {
						 aid = jq(this).val();
						jq.get(ajaxPrefix+"mvc/areaSlot?areaId="+ aid,function(data) {
							var data=data;
						getSlot(data);
						});
		             })
	...
	function bookNow() {
		...
		function Continue(){
			...
			if(cName=='' || cTotal=='' ||cBill=='' || bookedSlots=='' || timeTaken=='' || aid==''){
				...
				jq.ajax({
		            url:  ajaxPrefix+"mvc/slotBookingUsingWallet",
		            type: "POST",
		            dataType: "json",
